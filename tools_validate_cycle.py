...
根拠となる検査項目名を各行に付ける（ルール番号はテーマごとに振り直されるため番号では書かない。data/rules.md 参照）。
"""
...
def main(n, as_json=False):
    ...
    ACTIVE = [x['key'] for x in (s.get('candidates') or []) if isinstance(x, dict) and x.get('key') and x['key'] != '____']
    ...
    for r in rounds:
        ...
        for i, x in enumerate(r.get('statements', [])):
            if x.get('speaker') == 'guest' and i == len(r['statements']) - 1:
                add('WARN', '発言順', 'R%s: ゲストがラウンド末尾で、直後の委員の反応がない' % r.get('round'))
    if rounds and not any_double:
        add('WARN', '反論権', '二度発言するラウンドが1つもない（誰も反論されなかった＝予定調和の兆候）')
    if len(sizes) >= 3 and len(set(sizes)) == 1:
        add('WARN', '反論権', 'ラウンドの発言数が全て同じ (%d) — 機械的に埋めている疑い' % sizes[0])
    ...
    if d.get('agentMode'):
        ...
        if n_src < len(stmts) * 0.8:
            add('WARN', '出典', 'sources 付きの発言が %d/%d' % (n_src, len(stmts)))
        if n_nf == 0 and '維持' not in c.get('verdict', ''):
            add('WARN', '出典', '全員の newFinding が null なのに verdict が「前回維持」と言っていない')

    # ---- R24 字数 ----
    sl = c.get('stanceLabel', '')
    if len(sl) > 15:
        add('FAIL', '字数', 'stanceLabel が %d字（上限15）: %s' % (len(sl), sl[:30]))
    for i, w in enumerate(c.get('nextWatch', [])):
        if len(w) > 20:
            add('FAIL', '字数', 'nextWatch[%d] が %d字（上限20）' % (i, len(w)))
    for r in rounds:
        for w in r.get('watchItems', []):
            if len(w) > 20:
                add('WARN', '字数', 'R%s watchItems が %d字（上限20）' % (r.get('round'), len(w)))

    # ---- R10 baseline / incr2027 / R8 / R13 ranking (comparison モードのみ。candidates が無い／空ならスキップ) ----
    if ACTIVE:
        bl = c.get('baseline', {})
        for k in ACTIVE:
            v = bl.get(k)
            if not isinstance(v, dict) or v.get('price') is None:
                add('FAIL', '基準/算術', 'baseline.%s がない' % k)
            elif pf.get(k) and abs(v['price'] - pf[k]) / pf[k] > 0.03 and not bl.get('note'):
                add('WARN', '基準/株価', 'baseline.%s=%s が priceFeed=%s と3%%超ずれているのに note に理由がない' % (k, v['price'], pf[k]))
        if not c.get('rankingBasis'):
            add('FAIL', '基準/算術', 'rankingBasis がない')

        rk = c.get('ranking', [])
        keys = [r.get('key') for r in rk]
        if sorted(keys) != sorted(ACTIVE):
            add('FAIL', '順位', 'ranking の銘柄が %s（期待 %s）' % (keys, ACTIVE))
        ...
        for r in rk:
            ...
            for h, lab in ((h6, 'h2026'), (h7, 'h2027')):
                if not (h.get('bull', -999) > h.get('base', -999) > h.get('bear', -999)):
                    add('FAIL', '順位', '%s.%s が bull>base>bear になっていない' % (k, lab))
            if h6.get('base') is not None and h7.get('base') is not None:
                ...
                if not near(r.get('incr2027'), inc):
                    add('FAIL', '基準/算術', '%s.incr2027=%s（計算値 %.1f）' % (k, r.get('incr2027'), inc))
                ...
                if not near(A.get('h2026'), a6) or not near(A.get('h2027'), a7):
                    add('FAIL', '資金効率', '%s.annualized=%s（計算値 %.1f/%.1f）' % (k, A, a6, a7))
                G = r.get('gainPain') or {}
                for h, lab in ((h6, 'h2026'), (h7, 'h2027')):
                    if h.get('bear'):
                        g = h['base'] / abs(h['bear'])
                        if not near(G.get(lab), g, 0.05):
                            add('FAIL', '資金効率', '%s.gainPain.%s=%s（計算値 %.2f）' % (k, lab, G.get(lab), g))
            for f in ('rank2026', 'rank2027', 'rankEff2026', 'rankEff2027'):
                if r.get(f) is None:
                    add('FAIL', '順位/効率', '%s.%s がない' % (k, f))
            ...
            if pb is not None and nb is not None and pb > 0 and (pb - nb) / pb >= 0.30:
                ...
                if not any(w in rat for w in ('ガイダンス', '通期', '計画')) or not any(w in rat for w in ('高値', '年初来', 'ドローダウン')):
                    add('WARN', '切り下げ根拠', '%s: h2027 base を %s→%s に切り下げたが、rationale にガイダンス整合と直近高値整合の両方が見当たらない' % (k, pb, nb))
        if not c.get('opportunityCost'):
            add('FAIL', '資金効率', 'opportunityCost がない')

    # ---- R25 memberPositions ----
    mp = c.get('memberPositions')
    if mp is None:
        add('FAIL', '議長説明責任', 'memberPositions がない')
    else:
        got = {x.get('key') for x in mp}
        miss = sorted(set(team_then) - got)
        if miss:
            add('FAIL', '議長説明責任', 'memberPositions に %s がない' % ','.join(miss))
        rejected = [x for x in mp if x.get('adopted') is False]
        if not rejected:
            add('WARN', '議長説明責任', '全員 adopted:true — 食い違いを隠している疑い')
        for x in rejected:
            if not x.get('overrideReason'):
                add('FAIL', '議長説明責任', '%s は adopted:false なのに overrideReason がない' % x.get('key'))
        ...
        for x in r.get('statements', []):
            ...
            for k, tok in dnu:
                for m in re.finditer(r'(?<![\d,])' + re.escape(tok), t):   # 「6,110円」の中の「110円」に反応しない
                    ...
                    add('WARN', '確定値', 'R%s %s: doNotUse「%s」が現在の事実として使われている可能性: …%s…' % (r.get('round'), x.get('speaker'), tok, ctx.replace('\n', ' ')))
                    break
            for m in YEN.finditer(t):
                ...
                for k, p in pf.items():
                    if p and 0.75 * p <= v <= 1.25 * p and abs(v - p) / p > 0.03:
                        add('WARN', '株価検査', 'R%s %s: 「%s円」を現在値として提示（priceFeed.%s=%s と %.1f%% 乖離）' % (r.get('round'), x.get('speaker'), m.group(1), k, p, abs(v - p) / p * 100))
                        break
